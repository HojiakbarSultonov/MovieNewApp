...
function Auth() {
    ...
    if(user) router.push('/')
    if(isLoading) return <>Loading.</>   // ! qoyish kk
    ...
    return (
        <div className='relative flex h-screen w-screen flex-col md:items-center md:justify-center bg-black md:bg-transparent'>
            ...
            <Image src={'https://bit.ly/3JaHcJU'} alt={'bg'} fill className='object-cover -z-10 !hidden sm:!inline opacity-60' />
            ...
            <div >
                ...
                <Formik initialValues={{ email: '', password: '' }} onSubmit={onSubmit} validationSchema={validation}>
                    <Form className='relative mt-24 space-y-8 rounded bg-black/75 py-10 px-6 md:mt-0 md:max-w-md md:px-10'>
                        ...
                        <button type='submit' disabled={isLoading} className="w-full bg-[#E10856] py-3 font-semibold mt-4">{isLoading ? 'Loading...' : auth === 'signin' ? "Sign In" : "Sign Up"}</button>
                        ...
                        {auth === 'signin' ?
                            (
                                ...
                                <div className='text-[grey]'>
                                    ...
                                    <button type='button' className='text-white hover:underline' onClick={() => toggleAuth('signin')}>Sign in</button>
                                </div>
                            )}
                    </Form>
                </Formik>
